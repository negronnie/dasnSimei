...
class MovimentoFinanceiroRepositoryTest {
    ...
    void testMovimentoFinanceiro_quandoReceberId_deveRetornarMovimentoComID() {
        repository.save(mov);
        MovimentoFinanceiro movSalvo = repository.findById(mov.getId()).get();

        assertNotNull(movSalvo);
        assertEquals(movSalvo.getId(), mov.getId());
    }
    ...
    void testObterTotalAnual() {
        ...
        assertEquals(totalAno, mov.getValor());
    }
    ...
    void testObterTotalTrimestre() {
        ...
        assertEquals(totalTrimestre, mov.getValor());
    }
    ...
    void testFindMovimentoFinanceiroByDescricaoContainingIgnoreCase() {
        ...
        assertEquals(mov.getDescricao().contains(statement), movSalvos.get(0).getDescricao().contains(statement));
    }
}